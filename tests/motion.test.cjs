...
function run(reduced, supported = true) {
    ...
    const document = {body, hidden: false, querySelector(s) { return s.includes('project-work-1') ? hero : workflow; }, querySelectorAll(s) { return s.startsWith('main') ? [video] : [row]; }, addEventListener(n, cb) { events[n] = cb; }};
    const window = {innerHeight: 1000, matchMedia: () => preference, addEventListener(n, cb, options) {events[n] = cb; assert.equal(options.passive, true);}};
    ...
    assert.equal(row.children[1].styles.get('--reveal-delay'), '150ms');
    if (!supported) return;
    ...
    heroTop = -500;
    frameCallback();
    assert.equal(hero.styles.get('--hero-drift'), '32.5px');
    assert.equal(hero.styles.get('--hero-copy-drift'), '-16px');
